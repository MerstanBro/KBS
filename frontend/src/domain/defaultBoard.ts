import { GRID_SIZE_DEFAULT } from "./constants";
import type { Board } from "./types";

export const DEFAULT_CONFIG: Board = {
  meta: {
    warehouse_x: 3,
    warehouse_y: 2,
    start_x: 3,
...
export function createDefaultBoard(): Board {
  const board = structuredClone(DEFAULT_CONFIG);
  board.meta.grid_size = board.meta.grid_size ?? GRID_SIZE_DEFAULT;
  return board;
}
